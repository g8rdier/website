export default function Footer() {
  return (
    <div className="flex flex-col lg:flex-row justify-start items-start p-24 gap-32 bg-slate-800 text-slate-50">
      <div>
        <p>Imprint & Contact</p> <p>Imprint of legal entity</p>
        <p>Purpose UG (limited liability)</p>
        <p>Seidl-Kreuz-Weg 9485737 Ismaning</p>
      </div>
      <div>
        <p>Registered office of the company:</p>
        <p>Ismaning - HRB Munich 292052</p>
        <p>Tax number: 143/173/60143VAT</p>
        <p>ID No.: DE368116002Managing</p>
        <p>Director: Lukas Faber</p>
        <p>Contact (l.faber@purpose.hr)</p>
      </div>
      <div>
        <p>field for email to sign up for news concerning ß-phase or launch:</p>
        <p>“sign up here for updates & exclusive access to our beta phase</p>
        <p>program, where you can test our application for free.”</p>
      </div>
    </div>
  )
}
